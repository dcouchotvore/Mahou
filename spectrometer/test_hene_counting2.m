function test_hene_counting2

global PARAMS NICONST;
PARAMS.nShots = 10000;
PARAMS.dataSource = 0;
PARAMS.noiseGain = 1;
PARAMS.speed = 1200;
...
%% set DMA transfer
DAQmxSetDIDataXferMech(lib,hTask,chanName,NICONST.DAQmx_Val_DMA);


%% commit (make everything as ready to go as it can be)
DAQmxTaskControl(lib,hTask,NICONST.DAQmx_Val_Task_Commit);
...
options.bin_min = timeFsToBin(options.t_start,options.bin_zero)+1;
options.bin_max = timeFsToBin(options.t_end,options.bin_zero)-1;
